function [ABCD_chain,ABCD_cascade] = ABCD_uniform(R_min,R_max,G_min,G_max,B_min,B_max,N,M)

% Initialise matrix with zeros.  
ABCD_chain = zeros(2,2,N,M);

% Make distributions: Uniform Distributions
pd_R = makedist('Uniform','lower',R_min,'upper',R_max); % Create the Resistance distribution object
rng('default')  % Used for reproducibility
R = random(pd_R,[N,M]);

pd_G = makedist('Uniform','lower',G_min,'upper',G_max); % Create the Conductance distribution object
rng('default')  % Used for reproducibility
G = random(pd_G,[N,M]);

pd_B = makedist('Uniform','lower',B_min,'upper',B_max);  % Create the Susceptance distribution object
rng('default')  % Used for reproducibility
B = random(pd_B,[N,M]);
